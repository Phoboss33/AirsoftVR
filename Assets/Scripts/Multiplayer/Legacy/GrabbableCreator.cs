...
public class GrabbableCreator : NetworkBehaviour
{
    ...
    [SerializeField] Vector2 prefabSizes = new Vector2(0.01f, 2.0f);
    ...
    private void Start() {
        if (IsServer || IsHost) {
            for (int i =0; i < maxObjectsToSpawn; i++) {
                ...
                go.transform.position = new Vector3(Random.Range(placementArea.x, placementArea.y), 5, Random.Range(placementArea.x, placementArea.y));
